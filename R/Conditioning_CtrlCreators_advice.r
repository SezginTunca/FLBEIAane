...
create.advice.ctrl <- function(stksnames, HCR.models = NULL, HCR.ctrls = NULL,...){
    ...
    HCR.models.available <- c('fixedAdvice','annualTAC','IcesHCR','ghlHCR','annexIVHCR', 'froeseHCR', 'neaMAC_ltmp', 'aneHCRs')
    ...
    for(st in 1:length(stksnames)){
        res[[st]] <- list()
        res[[st]][['ass.year']]   <- 'all'
        res[[st]][['ass.curryr']] <- extra.args[[paste("ass.curryr",stksnames[st], sep = ".")]] # If assess stock status including the year in which we are carrying out the assessment
        res[[st]][['ass.season']] <- extra.args[[paste("ass.season",stksnames[st], sep = ".")]] # By default: assessment in the last year
...
create.annexIVHCR.ctrl <- function(resst,stkname, largs){
    ...
    resst <- c(resst, index = 1, ref.pts = NULL, type = 2, advice = "catch")
...
create.ghlHCR.ctrl <- function(resst,stkname, largs){
    ...
    resst <- c(resst, ref.pts = NULL, advice = "catch")
...
#                        *** create.aneHCRs.ctrl  ***
#-------------------------------------------------------------------------------
create.aneHCRs.ctrl <- function(resst,stkname, largs){
