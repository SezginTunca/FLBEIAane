...
create.fleets.ctrl <- function(fltsnames,  n.flts.stks, flts.stksnames, catch.threshold = NULL,  seasonal.share = NULL, 
    ...
    catch.models.available   <- c('CobbDouglasAge', 'CobbDouglasBio','seasonshare')
    ...
    # 'seasonshare' only valid for 'fixedEffort' effort model
    flst.ss <-  which( catch.models %in% 'seasonshare')
    flst.ef <- c(); for (i in 1:length(effort.models)) flst.ef <- c(flst.ef, rep(effort.models[i], n.flts.stks[i]))
    if (sum(flst.ef[flst.ss]!='fixedEffort')>0) stop("'seasonshare' catch model is only valid for 'fixedEffort' effort model") 
    # price models.
...
#-------------------------------------------------------------------------------
#                       ** create.SSFB.ctrl **
# extra args: restriction.fltname, effort.realloc.fltname, flq.fltname (effectiveDay.perc)
#-------------------------------------------------------------------------------
create.SSFB.ctrl <- function(resf, fltname,largs){
  
  
  # if NULL set default values
  rest       <- ifelse(is.null(largs[[paste('restriction', fltname,sep='.')]]), 'catch', largs[[paste('restriction', fltname,sep='.')]])
  if(is.null(largs[[paste('effectiveDay.perc', fltname,sep='.')]])) {
    flq.fl <- largs[[paste('flq', fltname,sep='.')]]
    if (is.null(flq.fl)) {
      effDayperc <- NULL
    } else {
      effDayperc <- FLQuant( 1, dim=dim(flq.fl), dimnames=dimnames(flq.fl))
    }
  } else {
    effDayperc <- largs[[paste('effectiveDay.perc', fltname,sep='.')]]
  }
  effrealloc <- ifelse(is.null(largs[[paste('effort.realloc', fltname,sep='.')]]), 'curr.eff', largs[[paste('effort.realloc', fltname,sep='.')]])

  # checkings
  if(rest != 'catch') stop("'", rest, "' restriction in 'SSFB' is not yet implemented.")
  if(is.null(effDayperc)) warning("You have selected 'SSFB' effort model for fleet '",  fltname, "'. 
                                  Thus, you have to fill 'effectiveDay.perc' FLQuant in fleets.ctrl[['", fltname,"']].\n", sep = "")
  if(effrealloc!='curr.eff') stop("'", effrealloc, "' function has not yet been implemented to set effort reallocation in 'SSFB'.")
  
  resf[['restriction']]       <- rest
  resf[['effectiveDay.perc']] <- effDayperc 
  resf[['effort.realloc']]    <- effrealloc
...
#-------------------------------------------------------------------------------
#                   ** create.seasonshare.ctrl **
# extra args: catch.dependence.fltname.stkname
#-------------------------------------------------------------------------------
create.seasonshare.ctrl <- function(resfst, fltname, stkname, largs) {

    cdep <- ifelse(is.null(largs[[paste('catch.dependence', fltname, stkname, sep='.')]]),  NA, largs[[paste('catch.dependence', fltname, stkname, sep='.')]])
    
    if (!is.na(cdep)) {
      resfst[["catch.dependence"]] <- cdep
    } else {
      warning( paste ("Not dependencies defined.\n If catches for fleet '", fltname, "' and stock '", stkname, "' depend on the catches of any other fleet,\n",
               "please set fleets.ctrl[['", fltname,"']][['", stkname,"']] = name of the fleet from wich same seasonal shares should be set.\n", sep = ""))
    }
