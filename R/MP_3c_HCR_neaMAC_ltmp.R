#-------------------------------------------------------------------------------
# HCR proposed in NEA MAC lont term managment plan
# Biomass Based HCR.
#  Reference Points: Btrigger, Blim and Fmsy.
#           - Btrigger = Bpa = 2 200 000 tons 
#           - Blim  = 1 670 000 tons
#           - Fmsy = 0.22
#
...
neaMAC_ltmp <- function(stocks, advice, advice.ctrl, year, stknm,...){
  ...
  # project the stock 3 years, (current year, TAC year, TAC year + 1 for ssb or biomass constraints).
   ...
   # if(dim(stk@m)[1] == 1)    stk@harvest[] <- stk@catch.n[]/stk@stock.n[] 
    ...
    if (sum(ref.pts[    'Fmsy',]!=0.22)>0) stop('Fmsy value should be 0.22')
    if (sum(ref.pts[    'Blim',]!=1670)>0) stop('Blim value should be 1 670 thousand tons')
    if (sum(ref.pts['Btrigger',]!=2200)>0) stop('Fmsy value should be 2 200 thousand tons')
