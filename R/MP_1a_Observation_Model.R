...
perfectObs <- function(biol, fleets, covars, obs.ctrl, year = 1, season = NULL, ...){
    ...
    ns <- dim(biol@n)[4]
    it <- dim(biol@n)[6]
    ss <- ifelse(is.null(season), ns, season)
    ...
    #
      ...
      # sum only along s<=ss
      res@m[,year-1,]      <- seasonSums(biol@m[,year-1,1,1:ss])
      res@m.spwn[,year-1,] <- seasonSums(biol@spwn[,year-1,1,1:ss])/length(1:ss)
    }
            ...
            for(a in ai:na){
                for(i in 1:it){
                    ...
                    if(n.[a,y,i] < c.[a,y,i]) res@harvest[a,y,,,,i] <- 10
                        ...
                        res@harvest[a,y,,,,i] <- ifelse(is.numeric(zz), zz, c(res@harvest[ai-1,y,,,,i]) )
                    }
                }
            }
        }
...
#-------------------------------------------------------------------------------    
# bioInd(biol, index, obs.ctrl, year)
# index aggregated in biomass.
#-------------------------------------------------------------------------------   
bioInd <- function(biol, index, obs.ctrl, year,...){
